...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  window_len = 2*skip_window+1
  
  no_of_windows = batch_size/num_skips
  i,num = 0,0

  while(i<no_of_windows):

    mid = data_index+window_len//2
    context_id = data[mid]

    start_point = mid - num_skips//2

    for j in range(start_point,start_point+num_skips+1):

      if j!=mid:
        labels[num,0] = data[j]
        batch[num] = context_id
        num+=1
